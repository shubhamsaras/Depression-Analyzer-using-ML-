...
with io.open("chrome_history.csv", 'r', encoding="utf8") as csvinput:
    ...
    #myList = [true, false]
        if (true >= 180):
            ...
            rate = 10
        f = open("extension/rating", "w+")
        f.write("Depression Rating : ")
        f.write(str(rate))
        f.close()
        f = open("extension/depression_number", "w+")
        f.write(str(true))
        f.close()
        f = open("extension/detailsDisplay", "w+")
        f.write("<div> Total depressive sentences :"+str(true)+"</div>")
        f.write("<div>Total non depressive sentences :"+str(false)+"</div>")
        f.close()
    csvoutput.close()
csvinput.close()
